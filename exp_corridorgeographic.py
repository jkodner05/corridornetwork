...
#                plt.setp(lines, color='lightgreen')
        ...
        lines = plt.plot(points)
        plt.setp(lines, color='blue', linewidth = 3, linestyle="--")
...
#                continue
            ...
            if i <= leng0:
                ...
                plt.setp(lines, color='deepskyblue', linestyle="--")
        axes = plt.gca()
        ...
        plt.setp(avgline1, color='blue', linewidth = 3, linestyle="--")
        for i in range(len(stages)-1):
            plt.plot((sum(stages[0:i+1]),sum(stages[0:i+1])), (0, 100), 'k:', linewidth = 3)

        plt.ylabel('Percent NCF', fontsize = 15)
...
#        plt.legend(handles=[get_label("red","On-Route Overall Avgs"),get_label("blue","Off-Route Overall Avgs"),get_label("salmon","On-Route Comm Avgs"),get_label("deepskyblue","Off-Route Comm Avgs"),mlines.Line2D([], [], linewidth=3, linestyle=':', color="black", label='Phase Boundary')], fontsize=10, loc="best")
        plt.legend(handles=[mlines.Line2D([], [], linewidth=3, linestyle='-', color="red", label='On-Route Overall Avgs'),mlines.Line2D([], [], linewidth=3, linestyle='--', color="blue", label='Off-Route Overall Avgs'),mlines.Line2D([], [], linewidth=3, linestyle='-', color="salmon", label='On-Route Comm Avgs'),mlines.Line2D([], [], linewidth=3, linestyle='--', color="deepskyblue", label='Off-Route Comm Avgs'),mlines.Line2D([], [], linewidth=3, linestyle=':', color="black", label='Phase Boundary')], fontsize=10, loc="best")
#        plt.title("Percent NCF across Iterations", fontsize=15)
        plt.tight_layout()
        plt.savefig(outf)
#        plt.show()
        plt.cla()
...
def corridor_geographic_migmigneut(duration3):
    stage1iters = 5
    stage2iters = 15
    stage3iters = duration3
    stage4iters = 25-duration3
    stage5iters = 20
    presprob=0.9
    alpha = 0.2
    allpoints = []
...
#    crosskfactor = 0.005
    ...
    plot_full_corridor(allpoints, "../plots/stlgeo_migmig_neut"+str(duration3), groupss, (stage1iters, stage2iters, stage3iters, stage4iters, stage5iters))
    plot_full_corridor(allpointsnew, "../plots/stlgeo_migmig_neut_youngonly"+str(duration3), groupss, (stage1iters, stage2iters, stage3iters, stage4iters, stage5iters))
...
def corridor_geographic_migdiffadv(crosskdivisor, beta):
    stage1iters = 5
    stage2iters = 15
    stage3iters = 20
    stage4iters = 25
...
#    Ts = [createT_advantage_alpha(0.3), createT_advantage_beta(0.7)]
    Ts = [createT_advantage_alpha(0.3), createT_advantage_beta(beta)]

    print "Creating Matrix 1"
    A1, sizes, NumOnR  = createA_STLCorridor(sdfactor, kfactor, crosskfactor/crosskdivisor, intracrosskfactor, True)
#    A1, sizes, NumOnR  = createA_STLCorridor(sdfactor, kfactor, crosskfactor/1, intracrosskfactor, True)
    print "Creating Matrix 2"
    A2, sizes, NumOnR  = createA_STLCorridor(sdfactor, kfactor, crosskfactor/crosskdivisor/10, intracrosskfactor, True)
 #   A2, sizes, NumOnR  = createA_STLCorridor(sdfactor, kfactor, crosskfactor/10, intracrosskfactor, True)
    print "Creating Matrix 3"
    A3, sizes, NumOnR  = createA_STLCorridor(sdfactor, kfactor, crosskfactor, intracrosskfactor, False)
    n = sum(sizes)
    ...
    plot_full_corridor(allpoints, "../plots/stlgeo_migdiff_adv"+str(crosskdivisor), groupss, (stage1iters, stage2iters, stage3iters,stage4iters))
...
corridor_geographic_migdiffadv(1000, 0.7)
corridor_geographic_migdiffadv(1, 0.5)
corridor_geographic_migmigneut(0)
corridor_geographic_migmigneut(10)
